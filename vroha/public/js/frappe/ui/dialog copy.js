...
import './field_group';
import 'frappe/public/js/frappe/dom.js';
...
frappe.ui.Dialog = class Dialog extends frappe.ui.FieldGroup {
	...
	make() {
		...
		this.make_head();
		this.modal_body = this.$wrapper.find(".message-body");
		...
		this.header = this.$wrapper.find(".card-body");
		this.footer = this.$wrapper.find(".card-footer");
		...
		if (this.minimizable) {
			this.header.find('h4').click(() => this.is_minimized && this.toggle_minimize());
	...
	get_minimize_btn() {
		return this.$wrapper.find(".modal-content .btn-modal-minimize");
	}
	...
	set_title(t) {
		this.$wrapper.find(".card-body h4").html(`<span></span>${t}`);
	}
	...
	toggle_minimize() {
		...
		this.header.find('h4').toggleClass('cursor-pointer');
	...
	add_custom_action(label, action, css_class=null) {
		this.footer.removeClass('hide');
		let action_button = $(`
			<button type="button" class="btn-secondary form-control text-center style2-input text-white fw-600 bg-dark border-0 p-0  ${css_class || ''}">
				${label}
			</button>
		`);
