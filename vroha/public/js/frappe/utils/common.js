// custom common file between desk and website
import md5 from 'md5';
frappe.avatar = function (img_size="w30") {
	let user_info;
	let full_name = frappe.get_cookie("full_name");
	user_info = {
		image: frappe.get_cookie("user_image"),
		fullname: full_name
	};

	return frappe.get_avatar(user_info.fullname, user_info.image, img_size);
};

frappe.get_avatar = function(title, image_url=null, img_size="w30") {
	if (image_url) {
		const image = (window.cordova && image_url.indexOf('http') === -1) ? frappe.base_url + image_url : image_url;
		return `<img src="${image}" alt="user" title="${title}" class="${img_size} d-inline-block"></img>`;
	} else {
        if(title != "Guest" || ""){
            let abbr =  frappe.get_abbr(title);
            return `<span class="text-current" title="${title}">${abbr}</span>`;

        }else{
            return `<i class="text-current ti-user fw-700 font-sm"></i>`;
        }        
