...
from vroha.shopping_cart.product_query import CustomProductQuery
...
def get_products_html_for_website(txt=None, field_filters=None, attribute_filters=None, start=0, item_group=None):
    ...
    if not items:
        html = frappe.render_template('vroha/www/all/not_found.html', {})
...
def get_html_for_items(items):
	html = []
	for item in items:
		html.append(frappe.render_template('vroha/www/all/item_row.html', {
			'item': item
		}))
	return html
